...
- **Re-verified 2026-09-19** against a scratch content page (built, checked, deleted — not committed) covering all three forms Phase 9 below once assumed needed client-side JS: plain `[[Title]]` resolves to a real `wikilink` anchor, `[[Title|Alias]]` resolves with the alias as link text, and a typo'd/nonexistent title correctly falls back to `.broken-link`. All three go through the same `single.html` regex-rewrite → `render-link.html` path above, build-time only, no JS. The one form that doesn't resolve is `[[path/to/note]]` (falls to `.broken-link`, since matching is by `Title` only) — but a corpus-wide grep found zero real uses of that form; the one path-style link that ever existed (`pulled-pork.md`'s spice-rub reference, noted above) was corrected to title form rather than taught to the resolver. Not worth building path-based matching for a form nothing uses.
...
- [x] **Scale recipe**: a 1×–5× slider (0.5 steps, no custom-value input — revised down from an initial preset-pills-plus-custom-input version, which also covered a ½× half-recipe case this range no longer does).
- [x] **Change units**: As written / Metric / Imperial / Kelvin, styled as real buttons (5px "checkbox, buttons, chip base" radius, per STYLE.md's spacing guide) rather than `.filter-pill`'s 14px pill shape — revised from an initial pill-styled version once the units control was asked to read as buttons specifically, a deliberate deviation from the filter-pill shape (though its accent-soft-fill active state is kept). Approximate, cooking-standard conversion factors (1 cup = 240 mL, 1 tbsp = 15 mL, 1 tsp = 5 mL, 1 oz = 28.35 g, 1 lb = 453.6 g), not lab-precise ones — consistent with this project's existing tolerance for ballpark numbers (`data/ingredient_prices.yaml`'s own approximation). Kelvin is a deliberate no-op: nothing in an ingredient list is a temperature, so it renders identically to "As written" rather than fabricating a nonsensical conversion.
- [x] **Metric magnitude renormalization**: scaling a metric quantity past 1000 switches its unit even when "As written" is selected, not only under an explicit Metric conversion — "1000g" reads as "1kg", "1500g" as "1.5kg", exact (no cross-system rounding). A range's two ends are forced into the *same* final unit (picked from the higher end) rather than each picking its own — otherwise a range straddling the 1000 line could render as a nonsensical "800–1.2 kg" with only one end labelled.
...
- ~~A site-wide `public/link-index.json` ... resolution source for wiki-link resolution below~~ — **obsolete, not building this.** Written to feed the client-side resolver below, which turned out to be unnecessary; see 6.2a's 2026-09-19 re-verification. No other consumer needs it.
...
- ~~Wiki-link styling for in-recipe cross-references... rainbow-hue technique~~ — **done already**, build-time: `.wikilink`/`.extlink`/`.broken-link` classes from `render-link.html`, styled from STYLE.md tokens in `custom.css`. See 6.2a.
- ~~Wiki-link *resolution*, upstream of the styling item above and solved client-side on purpose...~~ — **superseded, not building this.** This item's premise (goldmark's link parser swallows `[[...]]` before any render hook gets a turn, so resolution must happen client-side) was tested and true for the *passthrough-extension* approach it describes — but a different approach shipped the same day (6.2a, commit `cfa7ede`): rewrite `[[Title]]`/`[[Title|Alias]]` in `.RawContent` to standard `[text](<dest>)` syntax *before* goldmark parses it, in `single.html`, so `render-link.html` fires normally. Re-verified 2026-09-19: plain, alias, and broken-link-fallback forms all resolve correctly at build time already, site-wide (every page through `single.html`, not just recipes). No client JS needed. `[[path/to/note]]` form is the one real gap (falls to `.broken-link`) but has zero uses in the corpus — not worth a resolver for a form nothing uses.
- ~~Verify degrade-safe: JS disabled or failing...~~ — moot for wiki links specifically (no JS involved), still applies to `warped-food.js`'s other features whenever those are built.
...
- **Phase 9** is also two independent tracks. The spec/validator track only needs Phase 1's corpus to test against and can start any time after that. The `warped-food.js` track should follow Phase 6 for its remaining STYLE.md-dependent UI (cook mode, print view, etc.) — wiki-link styling no longer motivates this ordering, since that's resolved build-time in 6.2a, not a `warped-food.js` feature. It absorbs the two client-side stretch goals originally sketched in Phase 7, so do it instead of those, not in addition to them.
